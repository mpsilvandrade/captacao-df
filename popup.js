...
function preencherImovel(d){
  ...
  if($('f-apto'))$('f-apto').value=d.apto||'';
  if($('f-dias'))$('f-dias').value=d.diasPublicado||'';
  if($('f-suites'))$('f-suites').value=d.suites||'';
  if($('f-sol'))$('f-sol').value=d.sol||'';
  if($('f-financ'))$('f-financ').value=d.financiamento||'';
...
function salvarImovel(){
  ...
  const f={
    ...
    apto:$('f-apto')?$('f-apto').value.trim():'',
    dias:$('f-dias')?$('f-dias').value.trim():'',
    tipo:'Apartamento',fin:'Venda',
    val:$('f-val').value.trim(),area:$('f-area').value.trim(),
    qts:$('f-qts').value.trim(),
    suites:$('f-suites')?$('f-suites').value.trim():'',
    gar:$('f-gar').value.trim(),
    elev:$('f-elev').value,
    sol:$('f-sol')?$('f-sol').value.trim():'',
    financ:$('f-financ')?$('f-financ').value:'',
    cond:$('f-cond').value.trim(),
    m2:$('f-m2').value.trim(),
    ...
    chrome.scripting.executeScript({
      ...
      func:(dados,KEY_P,KEY_IDX)=>{
        try{
          ...
          if(prosps.find(p=>
            p.end&&p.end.toLowerCase()===(dados.end||'').toLowerCase()&&
            (p.apto||'').toLowerCase()===(dados.apto||'').toLowerCase()
          ))
